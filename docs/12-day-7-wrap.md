...
3. **Push the branch + draft PR** when you set up a GitHub remote. The branch `claude/quirky-nightingale-10cb2b` has 20+ commits ahead of `main`, all locally committed; no `origin` is configured yet.
...
- **SFTS_Fundraiser permset** — equivalent of SFTS_Advocate but for fundraising-side work, when SFTS has someone dedicated to fundraising.
...
Clean. All 20+ commits are on branch `claude/quirky-nightingale-10cb2b`. No `origin` remote configured (yet).
